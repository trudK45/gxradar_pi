...
ControlDialog::ControlDialog( gxradar_pi * ppi, wxWindow* parent)
...
void ControlDialog::OnNoXmitDialogClick(wxCommandEvent& event)
{
    pPlugIn->OnNoXmitDialogClicked();
}
...
RangeDialog::RangeDialog( gxradar_pi * ppi, wxWindow* parent)
...
void RangeDialog::RangeDialogShow()
{
    ...
    pPlugIn->m_gain_control_mode = g_scan_gain_mode;
...
NoiseDialog::NoiseDialog( gxradar_pi * ppi, wxWindow* parent)
...
void NoiseDialog::OnSeaClutterOnOffClick( wxCommandEvent& event )
{
    pPlugIn->SetSeaClutterOnOff(SeaClutterOnOff->GetSelection());
}
...
void NoiseDialog::OnRainClutterModeClick( wxCommandEvent& event )
{
    pPlugIn->SetRainClutterMode(RainClutterOnOff->GetSelection());
}
...
extern int g_scan_sea_clutter_on_off;
extern int g_scan_sea_clutter_mode;
extern int g_scan_sea_clutter_level;
extern int g_scan_rain_clutter_level;
extern int g_scan_rain_clutter_mode;
extern int g_scan_crosstalk_mode;

void NoiseDialog::NoiseDialogShow()
{
    pPlugIn->m_sea_clutter_on_off = g_scan_sea_clutter_on_off;
    SeaClutterOnOff->SetSelection(pPlugIn->m_sea_clutter_on_off);
	...
	pPlugIn->m_rain_clutter_mode = g_scan_rain_clutter_mode;
    RainClutterOnOff->SetSelection(pPlugIn->m_rain_clutter_mode);
...
DomeDialog::DomeDialog( gxradar_pi * ppi, wxWindow* parent)
...
SentryDialog::SentryDialog( gxradar_pi * ppi, wxWindow* parent)
...
SentryAlarmDialog::SentryAlarmDialog( gxradar_pi * ppi, wxWindow* parent)
...
NoXmitDialog::NoXmitDialog( gxradar_pi * ppi, wxWindow* parent)
    : NoXmitDialogBase( parent )
{
    pPlugIn = ppi;
    pParent = parent;

    this->Connect( wxEVT_MOVE, wxMoveEventHandler( NoXmitDialog::OnMove ) );

    Init();
}


NoXmitDialog::~NoXmitDialog( )
{
    this->Disconnect( wxEVT_MOVE, wxMoveEventHandler( NoXmitDialog::OnMove ) );
}

void NoXmitDialog::Init()
{
     pNXZTranSlider->SetValue(pPlugIn->m_noxmitzone_transparency * 100);
}

void NoXmitDialog::OnClose( wxCloseEvent& event )
{
    pPlugIn->OnNoXmitDialogClose();
    event.Skip();
}

void NoXmitDialog::OnSize( wxSizeEvent& event )
{
    wxSize p = GetSize();
    pPlugIn->SetNoXmitDialogSizeX(p.x);
    pPlugIn->SetNoXmitDialogSizeY(p.y);
    event.Skip();
}

void NoXmitDialog::OnMove ( wxMoveEvent& event )
{
    wxPoint p = GetPosition();
    pPlugIn->SetNoXmitDialogX(p.x);
    pPlugIn->SetNoXmitDialogY(p.y);
    event.Skip();
}


void NoXmitDialog::OnNoXmitZoneClick( wxCommandEvent& event )
{
    pPlugIn->SetNoXmitZoneMode(NoXmitZoneOnOff->GetSelection());
    pPlugIn->SetNoXmitZoneColor( m_colorpicker->GetColour());
}

void NoXmitDialog::OnNoXmitZoneOnOffClick( wxCommandEvent& event )
{
    pPlugIn->SetNoXmitZoneMode(NoXmitZoneOnOff->GetSelection());
}

void NoXmitDialog::OnUpdateStartAngle( wxSpinEvent& event )
{
    int vals = StartAngle->GetValue();
    StartAngle->SetValue(vals);

    pPlugIn->SetNoXmitStartAngle(vals);    
}


void NoXmitDialog::OnUpdateEndAngle( wxSpinEvent& event )
{
    int vale = EndAngle->GetValue();
    EndAngle->SetValue(vale);

    pPlugIn->SetNoXmitEndAngle(vale);
}


void NoXmitDialog::OnColorChanged( wxColourPickerEvent& event )
{
    pPlugIn->SetNoXmitZoneColor( m_colorpicker->GetColour());
}

void NoXmitDialog::OnUpdateTranSlider(wxScrollEvent &event)
{
    pPlugIn->m_noxmitzone_transparency = ((double)pNXZTranSlider->GetValue()) / 100.;
    pPlugIn->UpdateDisplayParameters();
}


void NoXmitDialog::OnNoXmitCloseClick( wxCommandEvent& event )
{
    pPlugIn->OnNoXmitDialogClose();
}


extern int g_scan_noxmitzone_mode;
extern int g_scan_noxmit_start_angle;
extern int g_scan_noxmit_end_angle;

void NoXmitDialog::NoXmitDialogShow()
{
    pPlugIn->m_noxmitzone_mode = g_scan_noxmitzone_mode;
    NoXmitZoneOnOff->SetSelection(pPlugIn->m_noxmitzone_mode);

    pPlugIn->m_noxmit_start_angle = g_scan_noxmit_start_angle;
    StartAngle->SetValue(pPlugIn->m_noxmit_start_angle);

    pPlugIn->m_noxmit_end_angle = g_scan_noxmit_end_angle;
    EndAngle->SetValue(pPlugIn->m_noxmit_end_angle);
