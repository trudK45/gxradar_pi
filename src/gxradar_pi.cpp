...
#include "gxradar_pi.h"
#include "gxradardialog.h"
#include "GxRadarUI.h"
#include "georef.h"
...
short          g_scan_gain_auto;
short          g_scan_gain_auto_mode;
int            g_scan_noxmitzone_mode;
int            g_scan_noxmit_start_angle;
int            g_scan_noxmit_end_angle;


double         g_hdt;
double         g_current_headings[NO_OF_RADIALS]= {0.0};
...
{
    return (opencpn_plugin *) new gxradar_pi(ppimgr);
}
...
gxradar_pi::gxradar_pi(void *ppimgr)
...
int gxradar_pi::Init(void)
{
    ...
    m_gain_level = 50;
    m_gain_control_mode = 0;

    // NoiseDialog
    m_sea_clutter_on_off = 0;
    m_sea_clutter_mode = 0;
    m_sea_clutter_level = 0;
    m_rain_clutter_level = 0;
    m_rain_clutter_mode = 0;
    m_crosstalk_mode = 0;
    ...
    m_timedtransmit_mode = 0;
    m_standby_minutes = 10;
    m_transmit_minutes = 1;

    // NoXmitDialog
    m_noxmitzone_transparency = .50;
    m_noxmitzone_mode = 0;
    m_noxmit_start_angle = 0;
    m_noxmit_end_angle = 10;

    g_updatemode = 0;
    g_scan_gain_level = 0;
    g_scan_gain_mode = 0;
    g_scan_sea_clutter_on_off = 0;
    g_scan_sea_clutter_mode = 0;
    g_scan_sea_clutter_level = 0;
    g_scan_rain_clutter_level = 0;
    g_scan_rain_clutter_mode = 0;
    g_scan_crosstalk_mode = 0;
    ...
    plogcontainer = new wxDialog(NULL, -1, _T("gxradar_pi Log"), wxPoint(0,0), wxSize(600,400),
    ...
    grLogMessage(_T("gxradar_pi log opened\n"));

    AddLocaleCatalog( _T("opencpn-gxradar_pi") );
    ...
    //    Create the thread for Multicast radar data response reception
    ...
    //    Create the thread for Multicast radar video reception
    m_mcrx_video = new MulticastSThread(&m_mutex, _T("239.254.2.0"),_T("50102"));
    if ( m_mcrx_video->Run() != wxTHREAD_NO_ERROR )
    {
        delete m_mcrx_video;
        m_mcrx_video = NULL;
    }
...
bool gxradar_pi::DeInit(void)
{
    ...
    if (m_pNoXmitDialog != NULL)
    {
        m_pNoXmitDialog->Hide();
        m_pNoXmitDialog->Close();
        delete m_pNoXmitDialog;
        m_pNoXmitDialog = NULL;
    }
...
int gxradar_pi::GetAPIVersionMajor()
{
    return MY_API_VERSION_MAJOR;
}

int gxradar_pi::GetAPIVersionMinor()
{
    return MY_API_VERSION_MINOR;
}

int gxradar_pi::GetPlugInVersionMajor()
{
    return PLUGIN_VERSION_MAJOR;
}

int gxradar_pi::GetPlugInVersionMinor()
{
    return PLUGIN_VERSION_MINOR;
}

wxBitmap *gxradar_pi::GetPlugInBitmap()
{
    return m_pdeficon;
}

wxString gxradar_pi::GetCommonName()
{
    return _T("GxRadar");
}

wxString gxradar_pi::GetShortDescription()
{
    return _("Garmin Radar PlugIn for OpenCPN");
}

wxString gxradar_pi::GetLongDescription()
{
    return _("Garmin Radar PlugIn for OpenCPN\n");
}

bool gxradar_pi::CheckHostAccessible(wxString &hostname)
{
    bool bret = false;
...
void gxradar_pi::SetDefaults(void)
{
...
int gxradar_pi::GetToolbarToolCount(void)
{
return 1;
}

void gxradar_pi::OnContextMenuItemCallback(int id)
{
    ...
    if (NULL == m_pNoXmitDialog)
    {
        m_pNoXmitDialog = new NoXmitDialog(this, m_parent_window);
        m_pNoXmitDialog->SetSize(m_NoXmit_dialog_x, m_NoXmit_dialog_y,
                                 m_NoXmit_dialog_sx, m_NoXmit_dialog_sy);
        m_pNoXmitDialog->Hide();
    }
    if (m_pNoXmitDialog->IsShown()) m_pNoXmitDialog->Hide();
}

void gxradar_pi::OnControlDialogClose()
{
    if (m_pControlDialog->IsShown())
        m_pControlDialog->Hide();
    SaveConfig();
...
void gxradar_pi::OnRangeDialogClicked()
{
...
void gxradar_pi::OnRangeDialogClose()
{
    if (m_pRangeDialog->IsShown())
        m_pRangeDialog->Hide();
    SaveConfig();
...
void gxradar_pi::OnNoiseDialogClicked()
{
...
void gxradar_pi::OnNoiseDialogClose()
{
    if (m_pNoiseDialog->IsShown())
        m_pNoiseDialog->Hide();
    SaveConfig();
...
void gxradar_pi::OnDomeDialogClicked()
{
...
void gxradar_pi::OnDomeDialogClose()
{
    if (m_pDomeDialog->IsShown())
        m_pDomeDialog->Hide();
    SaveConfig();
...
void gxradar_pi::OnSentryDialogClicked()
{
...
void gxradar_pi::OnSentryDialogClose()
{
    if (m_pSentryDialog->IsShown())
        m_pSentryDialog->Hide();
    SaveConfig();
...
void gxradar_pi::OnNoXmitDialogClicked()
{
    if (m_pNoXmitDialog->IsShown())
    {
        m_pNoXmitDialog->Hide();
    }
    else
    {
        m_pNoXmitDialog->SetSize(m_NoXmit_dialog_x, m_NoXmit_dialog_y,
                                 m_NoXmit_dialog_sx, m_NoXmit_dialog_sy);
        m_pNoXmitDialog->NoXmitDialogShow();
    }
}

void gxradar_pi::OnNoXmitDialogClose()
{
    if (m_pNoXmitDialog->IsShown())
        m_pNoXmitDialog->Hide();
    SaveConfig();
...
void gxradar_pi::OnSentryAlarmDialogClose()
{
...
void gxradar_pi::OnToolbarToolCallback(int id)
{
    if (m_bmaster)
    {
...
bool gxradar_pi::ChangeCheck()
{
    bool change = false;

    if (m_savescan_range != g_scan_range_meters)
    {
        m_savescan_range = g_scan_range_meters;
        for (int i= 0 ; i < n_ranges ; ++i)
        {
            if ((available_ranges[i] > (g_scan_range_meters-20)/1852.0)&&(available_ranges[i] < (g_scan_range_meters+20)/1852.0))
            {
                m_manual_range = i;
                break;
            }
    ...
    if (m_savescan_sea_clutter_on_off != g_scan_sea_clutter_on_off)
    {
        m_savescan_sea_clutter_on_off = g_scan_sea_clutter_on_off;
        change= true;
    }
    ...
    if (m_savescan_rain_clutter_mode != g_scan_rain_clutter_mode)
    {
        m_savescan_rain_clutter_mode = g_scan_rain_clutter_mode;
        change = true;
    }
    if (m_savescan_crosstalk_mode != g_scan_crosstalk_mode)
    {
        m_savescan_crosstalk_mode = g_scan_crosstalk_mode;
        change = true;
    }
    ...
    if (m_savescan_noxmitzone_mode != g_scan_noxmitzone_mode)
    {
        m_savescan_noxmitzone_mode = g_scan_noxmitzone_mode;
        change = true;
    }
    if (m_savescan_noxmit_start_angle != g_scan_noxmit_start_angle)
    {
        m_savescan_noxmit_start_angle = g_scan_noxmit_start_angle;
        change = true;
    }
    if (m_savescan_noxmit_end_angle != g_scan_noxmit_end_angle)
    {
        m_savescan_noxmit_end_angle = g_scan_noxmit_end_angle;
        change = true;
    }
...
bool gxradar_pi::RenderGLOverlay(wxGLContext *pcontext, PlugIn_ViewPort *vp)
{
    ...
    if (ChangeCheck())
    {
        ...
        if (m_pNoXmitDialog)
            if (m_pNoXmitDialog->IsShown())m_pNoXmitDialog->NoXmitDialogShow();
    }
        ...
        (!(!m_bmaster && (m_slave_display_mode == SLAVE_DISPLAY_OFF))))
    {
        ...
        if (m_noxmitzone_mode == 1)
            RenderRadarNoXmitZone(radar_center, h_scale_ppm , v_scale_ppm, vp);
...
void gxradar_pi::RenderRadarOverlay(wxPoint radar_center,double h_scale_ppm, double v_scale_ppm, PlugIn_ViewPort *vp)
{
    ...
    if (g_updatemode == 0)
    {
        // swept scan  scaling...each scan holds 1 radial
        double bytes_per_meter = g_current_scan_length_bytes / (double)g_scan_display_meters;
        ...
        RenderRadarBuffer(g_scan_buf, g_current_scan_length_bytes, g_scan_display_meters, rotation, g_current_headings);

    }
    else
    {
        // full scan           scaling...each scan holds 1 radial
        double bytes_per_meter = g_static_scan_length_bytes / (double)g_static_scan_meters;
        double h_scale_factor = h_scale_ppm / bytes_per_meter;
        double v_scale_factor = v_scale_ppm / bytes_per_meter;
        glScaled( h_scale_factor, v_scale_factor, 1. );
...
void gxradar_pi::RenderRadarBuffer(unsigned char *buffer, int buffer_line_length, int scan_meters, double rotation, double headings[])
{
    if (buffer == 0) return;
    m_zonealarm = 0;
    int maxrad = buffer_line_length;
    ...
    // NO_OFF_RADIALS = 1440
    for (int i=0 ; i < NO_OF_RADIALS ; i++)
    {
        unsigned char *packet_data = &buffer[i * buffer_line_length];
        double angle = double(i)/4 + headings[i] + rotation - 90.0;
        double checkangle = double(i)/4;
        ...
        // Processs every byte for one radial
        for (int k=0 ; k < maxrad ; k++)
        {
            ...
            if (data1 != 0)
            {
                ...
                glColor4ub(sred, sgrn, sblu, m_overlay_transparency*data1);
                draw_blob_gl(rad, 0.0, 1.0, 0.25, ca, sa);
...
void gxradar_pi::RenderRadarGuardZone(wxPoint radar_center,double h_scale_ppm, double v_scale_ppm, PlugIn_ViewPort *vp)
{
...
void gxradar_pi::RenderGuardZone(double rotation)
{
...
void gxradar_pi::draw_blob_gl( double radius, double blob_start, double blob_end, double arc_length, double ca, double sa)
{
...
void gxradar_pi::RenderRadarNoXmitZone(wxPoint radar_center,double h_scale_ppm, double v_scale_ppm, PlugIn_ViewPort *vp)
{
    double rotation = vp->rotation * 180/PI;
    glPushAttrib(GL_COLOR_BUFFER_BIT | GL_LINE_BIT | GL_HINT_BIT);      //Save state
    glEnable(GL_BLEND);
...
void gxradar_pi::RenderNoXmitZone(double rotation)
{

    int start = 0;
    int stop = 360;
    int zone_size = 10000;
    unsigned char red = m_noxmitzone_color.Red();
    unsigned char green = m_noxmitzone_color.Green();
    unsigned char blue = m_noxmitzone_color.Blue();
    unsigned char alpha = 255 * m_noxmitzone_transparency;

    glColor4ub(red, green, blue, alpha);

    start = m_noxmit_start_angle;
    stop  = m_noxmit_end_angle;
...
void gxradar_pi::SetCursorLatLon(double lat, double lon)
{
}

void gxradar_pi::SetNMEASentence( wxString &sentence )
{
    m_NMEA0183 << sentence;
...
void gxradar_pi::SetPositionFixEx(PlugIn_Position_Fix_Ex &pfix)
{
...
bool gxradar_pi::LoadConfig(void)
{
    wxFileConfig *pConf = (wxFileConfig *)m_pconfig;

    if (pConf)
    {
        pConf->SetPath ( _T( "/Plugins/GxRadar" ) );
        pConf->Read ( _T( "GxRadarOpMode" ),  &m_bmaster, 0 );
        pConf->Read ( _T( "GxRadarDisplayMode" ),  &g_updatemode, 0 );
        pConf->Read ( _T( "GxRadarTransparency" ),  &m_overlay_transparency, .50 );
        pConf->Read ( _T( "GxRadarLog" ),  &b_enable_log, true );
        ...
        m_NoXmit_dialog_sx = pConf->Read ( _T ( "NoXmitDialogSizeX" ), 157L );
        m_NoXmit_dialog_sy = pConf->Read ( _T ( "NoXmitDialogSizeY" ), 232L );
        m_NoXmit_dialog_x =  pConf->Read ( _T ( "NoXmitDialogPosX" ), 161L );
        m_NoXmit_dialog_y =  pConf->Read ( _T ( "NoXmitDialogPosY" ), 254L );
...
bool gxradar_pi::SaveConfig(void)
{
    wxFileConfig *pConf = (wxFileConfig *)m_pconfig;
    ...
    {
        pConf->SetPath ( _T ( "/Plugins/GxRadar" ) );
        pConf->Write ( _T ( "GxRadarOpMode" ), m_bmaster );
        pConf->Write ( _T ( "GxRadarDisplayMode" ), g_updatemode );
        pConf->Write ( _T ( "GxRadarTransparency" ), m_overlay_transparency );
        pConf->Write ( _T ( "GxRadarLog" ), b_enable_log );
        ...
        pConf->Write ( _T ( "NoXmitDialogSizeX" ),  m_NoXmit_dialog_sx );
        pConf->Write ( _T ( "NoXmitDialogSizeY" ),  m_NoXmit_dialog_sy );
        pConf->Write ( _T ( "NoXmitDialogPosX" ),   m_NoXmit_dialog_x );
        pConf->Write ( _T ( "NoXmitDialogPosY" ),   m_NoXmit_dialog_y );
...
void gxradar_pi::RadarTxOff(void)
{
    if (!m_bmaster)
        return;

    rad_ctl_pkt_9 pck;
    pck.packet_type = 0x919;
    pck.len1 = 1;
    pck.parm1 = 0;            // 0 for "off"
...
void gxradar_pi::RadarTxOn(void)
{
    if (!m_bmaster)
        return;
    ...
    pck.packet_type = 0x919;
    pck.len1 = 1;
    pck.parm1 = 1;            // 1 for "on"

    SendCommand((unsigned char *)&pck, sizeof(pck));
    grLogMessage(_T("TX On\n"));
}

void gxradar_pi::SendCommand(unsigned char *ppkt, unsigned int n_bytes)
{
...
void gxradar_pi::CacheSetToolbarToolBitmaps( int bm_id_normal, int bm_id_rollover)
{
    if ((bm_id_normal == m_sent_bm_id_normal) && (bm_id_rollover == m_sent_bm_id_rollover))
        return;                                                 // no change needed

    if ((bm_id_normal == -1) || (bm_id_rollover == -1))
    {          // don't do anything, caller's responsibility
...
void gxradar_pi::UpdateState(void)
{
    g_pseudo_tick++;
    ...
    case 5:
        ...
        break;
    case 6:
        scan_state =_T("Unknown 6");
        break;
    case 7:
        scan_state =_T("Unknown 7");
        break;
    case 8:
        scan_state =_T("Unknown 8");
        break;
    case 9:
        scan_state =_T("Unknown 9");
        break;
    case 10:
        scan_state =_T("Unknown 10");
        break;
    default:
    ...
    case RADAR_IN_TIMED_WARMUP:
        {
            if (g_warmup_timer != g_last_warmup_timer)
            {
                ...
                wxString str;
                str.Printf(_T("%4d"), g_warmup_timer);
...
void gxradar_pi::Select_Range(double range_nm)
{
    if (!m_bmaster)
        return;

    int range_parm_meters = (range_nm * 1852) + 0.5;

    rad_ctl_pkt_12 pck_12;
    pck_12.packet_type = 0x91e;
    pck_12.len1  = 4;
    pck_12.parm1 = range_parm_meters;
    SendCommand((unsigned char *)&pck_12, sizeof(pck_12));
...
void gxradar_pi::SetUpdateMode(int mode)
{
    g_updatemode = mode;
}

void gxradar_pi::SetOperatingMode(int mode)
{
    if (m_bscanner_accessible)
        m_bmaster = (mode == 0);
    else
...
void gxradar_pi::SetScanColor( wxColour col )
{
    m_scan_color = col;
}

void gxradar_pi::SetRangeControlMode(int mode)
{
    if (!m_bmaster)
    {
        m_range_control_mode = 0;
...
void gxradar_pi::SetManualRange(int mode)
{
    if (!m_bmaster)
    {
        m_range_control_mode = 0;
...
void gxradar_pi::SetGainControlMode(int mode)
{
    if (!m_bmaster)
    {
        if (m_pRangeDialog)
...
void gxradar_pi::SetGainLevel(int mode)
{
    ...
    pck_9.len1 = 1;
    pck_9.parm1 = 0;
    SendCommand((unsigned char *)&pck_9, sizeof(pck_9));

    rad_ctl_pkt_10 pck_10;
    pck_10.packet_type = 0x925;
    pck_10.len1 = 2;
    pck_10.parm1 = m_gain_level*100;
    SendCommand((unsigned char *)&pck_10, sizeof(pck_10));
...
void gxradar_pi::SetSeaClutterOnOff(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    m_sea_clutter_on_off = mode;

    rad_ctl_pkt_9 pck;
    pck.len1 = 1;

    pck.packet_type = 0x939;

    if (m_sea_clutter_on_off == 0)
    {
        pck.parm1 = 0;          //off
    }
    else
    {
        if (g_scan_sea_clutter_mode == 0)
        {
            pck.parm1 = 1;      //man
        }
        else
    ...
    pck.packet_type = 0x93c;
    SendCommand((unsigned char *)&pck, sizeof(pck));

    wxString msg;
    msg.Printf(_T("m_rain_clutter_on_off: %d \n"), m_sea_clutter_on_off);
    grLogMessage(msg);
}

void gxradar_pi::SetSeaClutterMode(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    if (m_sea_clutter_mode)
    {
        pck.parm1 = m_sea_clutter_mode -1;
        pck.packet_type = 0x93b;
        SendCommand((unsigned char *)&pck, sizeof(pck));
    }
    ...
    m_sea_clutter_on_off = 1;
}

void gxradar_pi::SetSeaClutterLevel(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    rad_ctl_pkt_9 pck1;
    pck1.packet_type = 0x939;
    pck1.len1 = 1;
    pck1.parm1 = 1;
    SendCommand((unsigned char *)&pck1, sizeof(pck1));

    // Set level
    m_sea_clutter_level = mode;

    rad_ctl_pkt_10 pck;
    pck.packet_type = 0x93a;
    pck.len1 = 2;
    pck.parm1 = m_sea_clutter_level*100;
    SendCommand((unsigned char *)&pck, sizeof(pck));
...
void gxradar_pi::SetRainClutterLevel(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    rad_ctl_pkt_9 pck_m;
    pck_m.packet_type = 0x933;
    pck_m.len1 = 1;
    pck_m.parm1 = 1;          //Rain Clutter Mode on

    SendCommand((unsigned char *)&pck_m, sizeof(pck_m));

    m_rain_clutter_level = mode;

    rad_ctl_pkt_10 pck;
    pck.packet_type = 0x934;
    pck.len1 = 2;
    pck.parm1 = m_rain_clutter_level*100;
...
void gxradar_pi::SetRainClutterMode(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    m_rain_clutter_mode = mode;

    rad_ctl_pkt_9 pck;
    pck.packet_type = 0x933;
    pck.len1 = 1;
    pck.parm1 = m_rain_clutter_mode;

    SendCommand((unsigned char *)&pck, sizeof(pck));

    wxString msg;
    msg.Printf(_T("m_rain_clutter_mode: %d \n"), m_rain_clutter_mode);
    grLogMessage(msg);
}

void gxradar_pi::SetCrosstalkMode(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    onebyte_ctl_pkt pck;
    pck.len1 = 1;
    pck.parm1 = m_crosstalk_mode; 
    pck.packet_type = 0x91b;
    SendCommand((unsigned char *)&pck, sizeof(pck));
...
void gxradar_pi::SetDomeOffset(int mode)
{
    if (!m_bmaster)
    {
        if (m_pDomeDialog)
    ...
    rad_ctl_pkt_12 pck;
    pck.packet_type = 0x930;
    pck.len1 = 4;
    pck.parm1 = m_dome_offset<<5;
...
void gxradar_pi::SetDomeSpeed(int mode)
{
    if (!m_bmaster)
    {
        if (m_pDomeDialog)
    ...
    rad_ctl_pkt_9 pck;
    pck.packet_type = 0x916;
    pck.len1 = 1;

    if (m_dome_speed == 0)
        pck.parm1 = 0;          // Low Speed
    else
        pck.parm1 = 2;          // High speed
...
void gxradar_pi::SetTimedTransmitMode(int mode)
{   if (!m_bmaster)
    {
        if (m_pSentryDialog)
            if (m_pSentryDialog->IsShown()) m_pSentryDialog->SentryDialogShow();
...
void gxradar_pi::SetStandbyMinutes(int mode)
{
    ...
    rad_ctl_pkt_10 pck;
    pck.packet_type = 0x943;
    pck.len1 = 2;
    pck.parm1 = 0;

    pck.parm1 = (unsigned char)m_standby_minutes*60;
...
void gxradar_pi::SetTransmitMinutes(int mode)
{
    if (!m_bmaster || !(m_timedtransmit_mode == 0))
    {
        if (m_pSentryDialog)
    ...
    rad_ctl_pkt_10 pck;
    pck.packet_type = 0x944;
    pck.len1 = 2;

    pck.parm1 = (unsigned char)m_transmit_minutes*60;
...
void gxradar_pi::SetGuardZoneMode(int mode)
{
    m_guardzone_mode = mode;
}

void gxradar_pi::SetOuterRange(int mode)
{
    m_outer_range = mode;
}

void gxradar_pi::SetInnerRange(int mode)
{
    m_inner_range = mode;
}

void gxradar_pi::SetPartialArcMode(int mode)
{
    m_partial_arc_mode = mode;
}

void gxradar_pi::SetStartAngle(int mode)
{
    m_arc_start_angle = mode;
}

void gxradar_pi::SetEndAngle(int mode)
{
    m_arc_end_angle = mode;
}

void gxradar_pi::SetGuardZoneColor( wxColour col )
{
    m_guardzone_color = col;
}

void gxradar_pi::SetNoXmitZoneColor( wxColour col )
{
    m_noxmitzone_color = col;
}

void gxradar_pi::SetNoXmitZoneMode(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    m_noxmitzone_mode = mode;

    rad_ctl_pkt_9 pck;
    pck.len1 = 1;
    pck.packet_type = 0x93f;
    pck.parm1 = m_noxmitzone_mode;

    SendCommand((unsigned char *)&pck, sizeof(pck));

    wxString msg;
    msg.Printf(_T("m_noxmitzone_mode: %d \n"), m_noxmitzone_mode );
    grLogMessage(msg);
}

void gxradar_pi::SetNoXmitStartAngle(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    m_noxmit_start_angle = mode;

    rad_ctl_pkt_12 pck;
    pck.len1 = 4;
    pck.packet_type = 0x940;
    if (m_noxmit_start_angle > 180)
    {
        pck.parm1 = (m_noxmit_start_angle-360) <<5;
    }
    else
    {
        pck.parm1 = m_noxmit_start_angle<<5;
    }
    ...
    wxString msg;
    msg.Printf(_T("m_noxmit_start_angle: %d \n"), m_noxmit_start_angle);
    grLogMessage(msg);
}

void gxradar_pi::SetNoXmitEndAngle(int mode)
{
    if (!m_bmaster)
    {
        if (m_pNoiseDialog)
    ...
    m_noxmit_end_angle = mode;

    rad_ctl_pkt_12 pck;
    pck.len1 = 4;
    pck.packet_type = 0x941;
    if (m_noxmit_end_angle > 180)
    {
        pck.parm1 = (m_noxmit_end_angle-360) <<5;
    }
    else
    {
        pck.parm1 = m_noxmit_end_angle<<5;
    }
...
void gxradar_pi::UpdateDisplayParameters(void)
{
    RequestRefresh(GetOCPNCanvasWindow());
}

bool gxradar_pi::RenderOverlay(wxDC &dc, PlugIn_ViewPort *vp)
{
    if (0 == m_bshown_dc_message)
    {
        ...
        //        ::wxMessageBox(message, _T("gxradar_pi message"), wxICON_INFORMATION | wxOK, GetOCPNCanvasWindow());

        m_bshown_dc_message = 1;

        wxMessageDialog dlg(GetOCPNCanvasWindow(),  message, _T("gxradar_pi message"), wxOK);
        dlg.ShowModal();
    }
    return false;
}

void gxradar_pi::ShowNoAccessMessage(void)
{
...
//Multcasttread for receiving scanner radar data 

MulticastSThread::MulticastSThread( wxMutex *pMutex, const wxString &IP_addr, const wxString &service_port)
:wxThread()
{
...
void* MulticastSThread::Entry()
{
    ...
    */
    ...
    m_sock->SetOption(IPPROTO_IP, IP_ADD_MEMBERSHIP, (const void *)&mreq, sizeof(mreq));
...
MulticastSThread::~MulticastSThread()
{
    delete m_sock;
}

void MulticastSThread::process_scan_buffer(void)
{
    ...
    case 0x998:
        {
            if ((g_radar_state == RADAR_TX_ACTIVE)||(g_radar_state == RADAR_TT_TX_ACTIVE))
            {
                ...
                {
                    free (g_scan_buf);
                    g_scan_buf = (unsigned char *)calloc(NO_OF_RADIALS * packet.scan_length_bytes, 1);

                    g_max_scan_length_bytes = packet.scan_length_bytes;
                }

                // Range change?
                if ((unsigned int) g_scan_range_meters != packet.range_meters)
                {
                    if (g_scan_buf)
                        memset(g_scan_buf, 0, g_max_scan_length_bytes * NO_OF_RADIALS);
                    g_sweep_count = 0;
                }

                g_scan_display_meters = packet.display_meters;
                g_scan_range_meters = packet.range_meters;
                // >= 0 g_scan_angle < 1440
...
/*					g_radar_data_packets_per_rev++;
                ...
                g_current_headings[g_scan_angle] = g_hdt;

                // if in full screen mode, take a snapshot of the scan data on last radial (359.75 deg)

                if ((g_updatemode == 1) && (g_scan_angle == LAST_RADIAL) && (g_sweep_count >= 2))
                {

                    if (packet.scan_length_bytes > g_static_max_scan_length_bytes)
                    {
                        free(g_static_buf);
                        g_static_buf = (unsigned char *)calloc(NO_OF_RADIALS * packet.scan_length_bytes, 1);
                        g_static_max_scan_length_bytes = packet.scan_length_bytes;
                    }

                    memcpy(g_static_buf, g_scan_buf, packet.scan_length_bytes * NO_OF_RADIALS);
                    g_static_scan_length_bytes = g_current_scan_length_bytes;
                    g_static_scan_meters = g_scan_display_meters;
...
//Multcasttread for receiving scanner status 
...
void* MulticastRXThread::Entry()
{
    ...
    */
    ...
    while (0 == n_rx_once)
    {
        ...
        if (m_sock->LastCount())
        {
            grLogMessage(_T("->gxradar_pi: First Packet Rx\n"));
            n_rx_once++;
    ...
    case 0x0939:
        {
            ...
            case 1:
                {
                    g_scan_sea_clutter_on_off = 1;
                    g_scan_sea_clutter_manual = 1;
                }
                break;
            case 2:
                {
                    g_scan_sea_clutter_on_off = 1;
                    g_scan_sea_clutter_manual = 0;
                }
                break;
            }
    ...
    case 0x093b:
        {
            ...
            {
                g_scan_sea_clutter_mode = packet.parm1 +1;
            }
    ...
    case 0x0940:
        {
            ...
            if (packet.parm1 < 0)
            {
                g_scan_noxmit_start_angle = (packet.parm1/32)+360;
            }
            else
            {
                g_scan_noxmit_start_angle = packet.parm1/32;;
            }
    ...
    case 0x0941:
        {
            ...
            if (packet.parm1 < 0)
            {
                g_scan_noxmit_end_angle = (packet.parm1/32)+360;
            }
            else
            {
                g_scan_noxmit_end_angle = packet.parm1/32;;
            }
