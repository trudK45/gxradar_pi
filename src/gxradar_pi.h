/******************************************************************************
* $Id: gxradar_pi.h,v 1.8 2010/06/21 01:54:37 bdbcat Exp $
*
* Project:  OpenCPN
* Purpose:  GRIB Plugin
...
#ifndef _GXRADARPI_H_
#define _GXRADARPI_H_
...
typedef struct
{
    ...
    unsigned short    parm4;
    unsigned char     parm5;
    unsigned char     parm6;
    unsigned short    parm7;
...
#define GXRADAR_TOOL_POSITION    -1          // Request default positioning of toolbar tool

class gxradar_pi :  public wxTimer, opencpn_plugin_18
{
public:
    gxradar_pi(void *ppimgr);
    ...
    void SetNoXmitZoneColor(wxColour col);
    void SetNoXmitZoneMode(int mode);
    void SetNoXmitStartAngle(int mode);
    void SetNoXmitEndAngle(int mode);
    ...
    void SetNoXmitDialogX(int x){ m_NoXmit_dialog_x = x;}
    void SetNoXmitDialogY(int y){ m_NoXmit_dialog_y = y;}
    void SetNoXmitDialogSizeX(int sx){ m_NoXmit_dialog_sx = sx;}
    void SetNoXmitDialogSizeY(int sy){ m_NoXmit_dialog_sy = sy;}
    ...
    int    m_noxmitzone_mode;
    int    m_noxmit_start_angle;
    int    m_noxmit_end_angle;
    double m_noxmitzone_transparency;


    int   m_savescan_range;
    int   m_savescan_gain_level;
    int   m_savescan_gain_mode;
    int   m_savescan_sea_clutter_on_off;
    int   m_savescan_sea_clutter_mode;
    int   m_savescan_sea_clutter_level;
    int   m_savescan_rain_clutter_mode;
    ...
    int   m_savescan_noxmitzone_mode;
    int   m_savescan_noxmit_start_angle;
    int   m_savescan_noxmit_end_angle;
...
private:
    ...
    void draw_blob_gl(double radius, double blob_start, double blob_end, double arc_length, double ca, double sa);
	 void RenderRadarNoXmitZone(wxPoint radar_center, double h_scale_ppm, double v_scale_ppm, PlugIn_ViewPort *vp);
	 void RenderNoXmitZone(double rotation);
    ...
    int                 m_NoXmit_dialog_sx, m_NoXmit_dialog_sy;
    int                 m_NoXmit_dialog_x, m_NoXmit_dialog_y;
...
class MulticastSThread: public wxThread
{
public:

    MulticastSThread(wxMutex *pMutex, const wxString &IP_addr, const wxString &service_port);
    ~MulticastSThread(void);
    virtual ExitCode Entry();

    void OnExit(void);

private:
    void process_scan_buffer(void);
